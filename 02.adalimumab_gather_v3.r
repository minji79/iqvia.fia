...
if (!requireNamespace("arrow", quietly = TRUE)) {
  install.packages("arrow", repos = "https://cloud.r-project.org", lib = user_lib)
}
...
normalize_cols <- function(dt, needed) {
  for (nm in intersect(needed, names(dt))) {
    set(dt, j = nm, value = as.character(dt[[nm]]))
  }
  dt
}
...
prod[, molecule_name := toupper(trimws(molecule_name))]

adali_ndcs <- unique(prod[grepl("ADALIMUMAB", molecule_name, fixed = TRUE), .(ndc = get(std$ndc))])
...
plan[, (std$model_type) := toupper(trimws(as.character(get(std$model_type))))]
...
# Check unmatched
if (anyNA(plan$plan_type)) {
  cat("⚠️ Unmatched plan types:\n")
  print(unique(plan[is.na(plan_type), get(std$model_type)]))
}
...
read_claims <- function(p) {
  ...
  if (!(std$ndc %in% names(dt))) return(data.table())
  setkeyv(dt, std$ndc)
  dt <- adali_ndcs[dt, on = "ndc", nomatch = 0L]

  if (std$plan_id %in% names(dt)) {
    setkeyv(dt, std$plan_id)
    dt <- plan[dt, on = "plan_id"]
  } else {
...
ord <- intersect(
  c("claim_id", "ndc", "plan_id", "model_type", "plan_type", "rjct_grp", "rjct_grp_lbl", "encnt_outcm_cd"),
  names(claims_adali)
)
