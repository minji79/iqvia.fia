...
/* for cohort2 ="filled after RJ/RV in 90days", need different denominator */
data figure2; set figure2; if cohort2 ne "filled at the index attempt"; run;
proc freq data=figure2; table dominant_payer_adj; run;
proc freq data=figure2; table molecule_name; run;
...
data filler_1; set input.id_index; if cohort2 ="filled at the index attempt"; run;
...
proc sql;
    ...
    select qtr_indicator,
		   ...
		   sum(case when molecule_name = "SEMAGLUTIDE" then 1 else 0 end) as n_filler_sema_dm,  /* instead of first_filled_molecule */
		   sum(case when molecule_name = "SEMAGLUTIDE (WEIGHT MANAGEMENT)" then 1 else 0 end) as n_filler_sema_obesity,
		   sum(case when molecule_name = "TIRZEPATIDE" then 1 else 0 end) as n_filler_tirz_dm,
		   sum(case when molecule_name = "TIRZEPATIDE (WEIGHT MANAGEMENT)" then 1 else 0 end) as n_filler_tirz_obesity,
...
data filler_1; set filler_1; 
	qtr_indicator = intnx('quarter', first_filled_date, 0, 'beginning');
	filler =1; 
	format qtr_indicator yyq6.;
run;
proc sort data=filler_1; by qtr_indicator; run;
proc sql;
    create table filler_1_wide as
    select qtr_indicator,
               count(*) as n_filled_at_first
    from filler_1 
    group by qtr_indicator;
quit;
...
proc sql;
 	create table figure2_total as
	select distinct a.*, b.n_filled_at_first
	from figure2_total as a
	left join filler_1_wide as b
	on a.qtr_indicator =b.qtr_indicator; 
quit;
...
data figure2_total; set figure2_total; 
	...
	*pct_filler_at_first_total = n_filled_at_first / n_attempter_total * 100;
...
proc sgplot data=figure2_total;
    ...
    series x=qtr_indicator y=pct_filler_total / 
        ...
        legendlabel="Primary adherence (%)"
        name="line1";
	
    /* Second Line: Dotted/Dashed Blue */
    series x=qtr_indicator y=pct_filler_at_first_total / 
...
proc sgplot data=figure2_total;
	...
	*title "Post-rejection Filled Rate (%)";
	...
	*series x=qtr_indicator y=pct_filler_unclear / legendlabel="Unclear" lineattrs=(color=black);
    ...
    yaxis label="Filled Rate (%)" min=0 max=80 grid;
	*yaxis label="Post-rejection Filled Rate (%)" min=0 max=80 grid;
run;
...
/*============================================================*
 | 6. Figure 2 - % of filler / attempter by yearly
 *============================================================*/
/* total & by payers & by initially ordered products */
proc contents data=input.id_index; run;
*quarter indicator;
...
data filler_1; set filler_1; 
	...
	format qtr_indicator yyq6.;
run;
proc sort data=filler_1; by year; run;
proc sql;
    create table filler_1_wide as
    select year,
               count(*) as n_filled_at_first
...
run;
proc print data= figure2_total; var year pct_filler_total total_lcl total_ucl 
									pct_filler_medicare medicare_lcl medicare_ucl 
									pct_filler_commercial pct_filler_medicaid pct_filler_exchange pct_filler_unclear
									pct_filler_sema_dm pct_filler_sema_obesity pct_filler_tirz_dm pct_filler_tirz_obesity; run;
...
/* Primary adherence & Filled Rate at the first attempt  (%) */
proc sgplot data=figure2_total;
    title "Primary adherence and Filled Rate at the first attempt (%)";
    ...
    /* Second Line: Dotted/Dashed Blue */
    series x=year y=pct_filler_at_first_total / 
        lineattrs=(color=blue thickness=3 pattern=shortdash) /* pattern=2 or pattern=dot also work */
        markerattrs=(symbol=trianglefilled size=10 color=blue)
        legendlabel="Filled Rate at the first attempt (%)"
        name="line2";
    ...
    yaxis label="Filled Rate (%)" min=20 max=80 grid;
    
    /* Ensure the legend points to both names */
    keylegend "line1" "line2" / location=outside position=bottom;
run;


/* Post-rejection Filled Rate (%) */
proc sgplot data=figure2_total;
    title "Post-rejection Filled Rate (%)";
    ...
    xaxis label="Year" grid valuesrotate=diagonal;
    yaxis label="Post-rejection Filled Rate (%)" min=20 max=80 grid;
...
/*============================================================*
 | 6. Figure 2 - % of filler / attempter with 3-month moving averages
 *============================================================*/
/* monthly */
data figure2; set input.id_index; 
    month_indicator = intnx('month', index_rx_dt, 0, 'beginning');
	attempter =1;
    format month_indicator monyy7.;
run;
proc sort data=figure2; by month_indicator; run;
proc sql;
    create table figure2_wide as
    select month_indicator,
...
data filler; 
    set filler;
    month_indicator = intnx('month', first_filled_date, 0, 'beginning');
    filler = 1; 
    format month_indicator monyy7.;
run;
proc sort data=filler; by month_indicator; run;
proc sql;
    create table filler_wide as
    select month_indicator,
           sum(case when dominant_payer_adj = 'Commercial'        then 1 else 0 end) as n_filler_commercial,
...
proc sql;
	...
	from figure2_wide as a
	left join filler_wide as b
	on a.month_indicator =b.month_indicator; 
quit;
...
proc sort data=figure2_total; by month_indicator; run;

proc expand data=figure2_total out=figure2_smoothed method=none;
    id month_indicator;
    /* Create 3-month moving averages for each percentage variable */
	convert pct_filler_total      = ma_total      / transformout=(movave 3);
	
    convert pct_filler_commercial = ma_commercial / transformout=(movave 3);
    convert pct_filler_medicare   = ma_medicare   / transformout=(movave 3);
    convert pct_filler_medicaid   = ma_medicaid   / transformout=(movave 3);
    convert pct_filler_exchange   = ma_exchange   / transformout=(movave 3);
    convert pct_filler_unclear    = ma_unclear    / transformout=(movave 3);
	
    convert pct_filler_sema_dm      = ma_sema_dm      / transformout=(movave 3);
    convert pct_filler_sema_obesity = ma_sema_obesity / transformout=(movave 3);
    convert pct_filler_tirz_dm      = ma_tirz_dm      / transformout=(movave 3);
    convert pct_filler_tirz_obesity = ma_tirz_obesity / transformout=(movave 3);
run;
...
/* Figure for by Payer: Monthly 3-Month Moving Average */

/* figure for total */
/* Figure for Total: Monthly Counts + 3-Month Moving Average */
proc sgplot data=figure2_smoothed;
    title "GLP-1 RA Attempters and Fill Success Rate";
    title2 "Monthly Volume with 3-Month Moving Average Percentage";
    
    /* Monthly Bars for Volume (Using raw counts from the dataset) */
    vbarbasic month_indicator / response=n_attempter_total 
        fillattrs=(color=grey) 
        legendlabel="Monthly Attempters"
        name="bar";

    /* 3-Month Moving Average Line (Secondary Axis) */
    series x=month_indicator y=ma_total / 
        y2axis 
        lineattrs=(color=red thickness=3) 
        markerattrs=(symbol=circlefilled size=7 color=red)
        legendlabel="Fill Rate (3-Mo Moving Avg)"
        name="line";

    /* Axis Formatting */
    xaxis label="Month" grid valuesrotate=vertical;
    yaxis label="Number of Patients (Counts)" grid;
    
    /* Secondary Y-axis for the percentage */
    y2axis label="Fill Rate (%)" min=0 max=100 grid;
...
proc sgplot data=figure2_smoothed;
    ...
    /* Using the MA variables created in the PROC EXPAND step */
    series x=month_indicator y=ma_medicare / 
        legendlabel="Medicare D" 
        lineattrs=(color=blue thickness=2);

    series x=month_indicator y=ma_medicaid / 
        legendlabel="Medicaid" 
        lineattrs=(color=green thickness=2);

    series x=month_indicator y=ma_commercial / 
        legendlabel="Commercial" 
        lineattrs=(color=red thickness=2);

    series x=month_indicator y=ma_exchange / 
        legendlabel="Exchange" 
        lineattrs=(color=orange thickness=2);

    /* Formatting the Axes */
    xaxis label="Month" grid valuesrotate=vertical;
    yaxis label="3-Month Moving Average Fill Rate (%)" 
          grid min=0 max=100;
...
/* Figure for by initially ordered products */
proc sgplot data=figure2_smoothed;
    title "3-Month Moving Average: Fill Rate (%) by Molecule and Indication";

    /* Semaglutide Lines */
    series x=month_indicator y=ma_sema_dm / 
        legendlabel="Semaglutide (T2DM)" lineattrs=(color=blue thickness=2);
    series x=month_indicator y=ma_sema_obesity / 
        legendlabel="Semaglutide (Obesity)" lineattrs=(color=green thickness=2);

    /* Tirzepatide Lines */
    series x=month_indicator y=ma_tirz_dm / 
        legendlabel="Tirzepatide (T2DM)" lineattrs=(color=red thickness=2);
    series x=month_indicator y=ma_tirz_obesity / 
        legendlabel="Tirzepatide (Obesity)" lineattrs=(color=orange thickness=2);

    xaxis label="Month" grid valuesrotate=vertical;
    yaxis label="3-Month Moving Average Fill Rate (%)" min=0 max=100 grid;
run;
